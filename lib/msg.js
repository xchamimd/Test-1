...
// Updated function definition to use xCHAMi
const sms = (xCHAMi, m) => {
	if (m.key) {
		m.id = m.key.id
        ...
        // Updated xCHAMi usage
		m.sender = m.fromMe ? xCHAMi.user.id.split(':')[0]+'@s.whatsapp.net' : m.isGroup ? m.key.participant : m.key.remoteJid
	}
	if (m.message) {
		...
		if (m.msg) {
			...
			if (m.quoted) {
                ...
                // Updated xCHAMi usage
				m.quoted.fromMe = m.quoted.sender.split('@')[0].includes(xCHAMi.user.id.split(':')[0])
                ...
                // Updated xCHAMi usage
				m.quoted.delete = () => xCHAMi.sendMessage(m.chat, { delete: m.quoted.fakeObj.key })
                // Updated xCHAMi usage
				m.quoted.react = (emoji) => xCHAMi.sendMessage(m.chat, { react: { text: emoji, key: m.quoted.fakeObj.key } })
			}
		}
		m.download = (filename) => downloadMediaMessage(m, filename)
	}
	
    // Updated xCHAMi usage in all reply functions below
	m.reply = (teks, id = m.chat, option = { mentions: [m.sender] }) => xCHAMi.sendMessage(id, { text: teks, contextInfo: { mentionedJid: option.mentions } }, { quoted: m })
	m.replyS = (stik, id = m.chat, option = { mentions: [m.sender] }) => xCHAMi.sendMessage(id, { sticker: stik, contextInfo: { mentionedJid: option.mentions } }, { quoted: m })
	m.replyImg = (img, teks, id = m.chat, option = { mentions: [m.sender] }) => xCHAMi.sendMessage(id, { image: img, caption: teks, contextInfo: { mentionedJid: option.mentions } }, { quoted: m })
	m.replyVid = (vid, teks, id = m.chat, option = { mentions: [m.sender], gif: false }) => xCHAMi.sendMessage(id, { video: vid, caption: teks, gifPlayback: option.gif, contextInfo: { mentionedJid: option.mentions } }, { quoted: m })
	m.replyAud = (aud, id = m.chat, option = { mentions: [m.sender], ptt: false }) => xCHAMi.sendMessage(id, { audio: aud, ptt: option.ptt, mimetype: 'audio/mpeg', contextInfo: { mentionedJid: option.mentions } }, { quoted: m })
	m.replyDoc = (doc, id = m.chat, option = { mentions: [m.sender], filename: 'undefined.pdf', mimetype: 'application/pdf' }) => xCHAMi.sendMessage(id, { document: doc, mimetype: option.mimetype, fileName: option.filename, contextInfo: { mentionedJid: option.mentions } }, { quoted: m })
	m.replyContact = (name, info, number) => {
		var vcard = 'BEGIN:VCARD\n' + 'VERSION:3.0\n' + 'FN:' + name + '\n' + 'ORG:' + info + ';\n' + 'TEL;type=CELL;type=VOICE;waid=' + number + ':+' + number + '\n' + 'END:VCARD'
		xCHAMi.sendMessage(m.chat, { contacts: { displayName: name, contacts: [{ vcard }] } }, { quoted: m })
	}
    // Updated xCHAMi usage
	m.react = (emoji) => xCHAMi.sendMessage(m.chat, { react: { text: emoji, key: m.key } })
